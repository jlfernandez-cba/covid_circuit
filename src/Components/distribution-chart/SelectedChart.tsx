...
function SelectedChart() {
    ...
    if (!!busy) {
        ...
        return <ResponsiveContainer width="100%" aspect={1.8}><div style={{ width: "100%", height: "100%"}}><span>Reading data from server...</span></div></ResponsiveContainer>;
    } else if (!!error) {
        return <ResponsiveContainer width="100%" aspect={1.8}><div style={{ width: "100%", height: "100%"}}><span>Error reading data from server.</span></div></ResponsiveContainer>;
    } else {
        return (
            <LineageChart data={data} />
        );
    }
}
